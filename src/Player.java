...
public class Player {
    ...
    int[] chunkIn=new int[2];
    boolean thirdPerspective=true;
    int[] cubeIn=new int[3];
    private int yPosCount=0;
    private final double[] yPosHistoric=new double[10];
    ...
    Player(double positionX, double positionY, double positionZ,GameGrid gameGrid){
        this.gameGrid=gameGrid;
        xPosition=positionX;
        yPosition=positionY;
        zPosition=positionZ+10;
        setCubeAndChunkIn();
        //newChunkAround();
        SMH=new SpecialMoveHandler(gameGrid);
    }
    ...
    public  void respawn() {
        ...
        gameGrid.projectileContainer.Projectiles =new ArrayList<Projectile>();
    ...
    private void newChunkAround(){
        gameGrid.cubeContainer.CreateNewChunks();
    }
    ...
    try {
        ...
        if (gameGrid.cubeContainer.chunks[info1[0]+CubeContainer.numOfChunkX][info2[0]+CubeContainer.numOfChunkY].cubePositions[info1[1]][info2[1]][cubeIn[2]]) return true;
        if (gameGrid.cubeContainer.chunks[info1[0]+CubeContainer.numOfChunkX][info2[0]+CubeContainer.numOfChunkY].cubePositions[info1[1]][info2[1]][cubeIn[2] + 1]) return true;
        if (gameGrid.cubeContainer.chunks[info1[0]+CubeContainer.numOfChunkX][info2[0]+CubeContainer.numOfChunkY].cubePositions[info1[1]][info2[1]][cubeIn[2] + 2]) return true;
    ...
    private void setNewPositions(double deltaTime){
        ...
        if(isMovingRight) {
            yVelocity+=speed*multiplierOfSpeed*Math.sin(GameGrid.angleForHorizontalRotation);
            xVelocity+=speed*multiplierOfSpeed*Math.cos(GameGrid.angleForHorizontalRotation);
        }
        if(isMovingLeft){
            yVelocity-=speed*multiplierOfSpeed*Math.sin(GameGrid.angleForHorizontalRotation);
            xVelocity-=speed*multiplierOfSpeed*Math.cos(GameGrid.angleForHorizontalRotation);
        ...
        if(isMovingBackward){
            yVelocity+=speed*multiplierOfSpeed*Math.cos(GameGrid.angleForHorizontalRotation);
            xVelocity-=speed*multiplierOfSpeed*Math.sin(GameGrid.angleForHorizontalRotation);
        }
        if(isMovingForward) {
            yVelocity-=speed*multiplierOfSpeed*Math.cos(GameGrid.angleForHorizontalRotation);
            xVelocity+=speed*multiplierOfSpeed*Math.sin(GameGrid.angleForHorizontalRotation);
    ...
    private void detectionCollisionWithContext(){
        ...
        boolean cubeFront=(gameGrid.cubeContainer.chunks[xChunkNum+CubeContainer.numOfChunkX][yChunkNumFront+CubeContainer.numOfChunkY].cubePositions[newXpos][newYposFront][cubeIn[2]]
                ||gameGrid.cubeContainer.chunks[xChunkNum+CubeContainer.numOfChunkX][yChunkNumFront+CubeContainer.numOfChunkY].cubePositions[newXpos][newYposFront][cubeIn[2]+1]
                ||gameGrid.cubeContainer.chunks[xChunkNum+CubeContainer.numOfChunkX][yChunkNumFront+CubeContainer.numOfChunkY].cubePositions[newXpos][newYposFront][(int)(cubeIn[2]+height+.5)]);
        boolean cubeBack=(gameGrid.cubeContainer.chunks[xChunkNum+CubeContainer.numOfChunkX][yChunkNumBack+CubeContainer.numOfChunkY].cubePositions[newXpos][newYposBack][cubeIn[2]]
                ||gameGrid.cubeContainer.chunks[xChunkNum+CubeContainer.numOfChunkX][yChunkNumBack+CubeContainer.numOfChunkY].cubePositions[newXpos][newYposBack][cubeIn[2]+1]
                ||gameGrid.cubeContainer.chunks[xChunkNum+CubeContainer.numOfChunkX][yChunkNumBack+CubeContainer.numOfChunkY].cubePositions[newXpos][newYposBack][(int)(cubeIn[2]+height+.5)]);
        boolean cubeRight=(gameGrid.cubeContainer.chunks[XChunkNumRight+CubeContainer.numOfChunkX][YChunkNum+CubeContainer.numOfChunkY].cubePositions[newXposRight][newYpos][cubeIn[2]])||
                xVelocity>0&&(gameGrid.cubeContainer.chunks[XChunkNumRight+CubeContainer.numOfChunkX][YChunkNum+CubeContainer.numOfChunkY].cubePositions[newXposRight][newYpos][cubeIn[2]+1])||
                xVelocity>0&&(gameGrid.cubeContainer.chunks[XChunkNumRight+CubeContainer.numOfChunkX][YChunkNum+CubeContainer.numOfChunkY].cubePositions[newXposRight][newYpos][(int)(cubeIn[2]+height+.5)]);
        boolean cubeLeft=xVelocity<0&&(gameGrid.cubeContainer.chunks[XChunkNumLeft+CubeContainer.numOfChunkX][YChunkNum+CubeContainer.numOfChunkY].cubePositions[newXposLeft][newYpos][cubeIn[2]])||
                xVelocity<0&&(gameGrid.cubeContainer.chunks[XChunkNumLeft+CubeContainer.numOfChunkX][YChunkNum+CubeContainer.numOfChunkY].cubePositions[newXposLeft][newYpos][cubeIn[2]+1])||
                xVelocity<0&&(gameGrid.cubeContainer.chunks[XChunkNumLeft+CubeContainer.numOfChunkX][YChunkNum+CubeContainer.numOfChunkY].cubePositions[newXposLeft][newYpos][(int)(cubeIn[2]+height+.5)]);
    ...
    private void detectionCollisionWithEnemy(double deltaTime){
        ...
        if(pushCount>=pushTime){
            for(int i = 0; i< gameGrid.entityContainer.entities.size(); i++){
                Entity entity=gameGrid.entityContainer.entities.get(i);
                if(zPosition<entity.getzPosition()+ entity.getHeight()&&zPosition+height> entity.getzPosition()) {
                    if (Math.sqrt(Math.pow(entity.getxPosition() - xPosition, 2) + Math.pow(entity.getyPosition() - yPosition, 2)) < (entity.getWidth() / 2 + width / 2)) {
                        xPosition -= Math.cos(entity.angleWithPlayer) * entity.getSpeed() * 200 * deltaTime;
                        xVelocity = -Math.cos(entity.angleWithPlayer) * entity.getSpeed() * 200 * deltaTime;
                        yPosition -= Math.sin(entity.angleWithPlayer) * entity.getSpeed() * 200 * deltaTime;
                        yVelocity = -Math.sin(entity.angleWithPlayer) * entity.getSpeed() * 200 * deltaTime;
                        detectionCollision(3);
                        detectionCollision(4);
                        detectionCollision(5);
                        detectionCollision(6);
                        Stats.health -= entity.strength;
                        pushCount = 0;
                    }
                }
            }
    ...
    private void projectileCollisionHandler(double deltaTime){
        for(var i = 0; i< gameGrid.projectileContainer.Projectiles.size(); i++){
            Projectile projectile=gameGrid.projectileContainer.Projectiles.get(i);
            if(!projectile.isFriendly() ) {
                if (detectionCollisionWithProjectile(projectile.getxPosition(), projectile.getyPosition(), projectile.getzPosition(), projectile.getSize())) {
                    yVelocity = projectile.getyVelocity()  * deltaTime*2;
                    xVelocity = projectile.getxVelocity()  * deltaTime*2;
                    yPosition += projectile.getyVelocity()  * deltaTime*2;
                    xPosition += projectile.getxVelocity()  * deltaTime*2;
                    detectionCollision(3);
                    detectionCollision(4);
                    detectionCollision(5);
                    detectionCollision(6);
                    Stats.health-=projectile.getDamage();
